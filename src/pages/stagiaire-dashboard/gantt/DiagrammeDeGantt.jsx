...
export default function DiagrammeDeGantt() {
      ...
      try {
        ...
        const tasks = [
          // Parent task
          {
            ...
            duration: Math.ceil(
              (new Date(data.dateFin) - new Date(data.dateDebut)) / 86400000
            ),
            open: true,
          },
          // Child tasks
          ...data.tacheDTOList.map((tache) => ({
            ...
            duration: tache.duree.includes("semaine")
              ? parseInt(tache.duree) * 7
              : parseInt(tache.duree) || 1,
            parent: -1,
  ...
  const generateGanttImage = async () => {
    ...
    try {
      ...
      const height =
        config.margin.top +
        (config.barHeight + config.rowGap) * taskCount +
      ...
      ctx.fillText(
        `Diagramme de Gantt - ${ganttData.titre}`,
        config.margin.left,
        30
      );
      ...
      const parentDuration = Math.ceil(
      ...
      ganttData.tacheDTOList.forEach((tache) => {
        ...
        const startOffset = Math.max(
          0,
          Math.ceil((startDate - dateDebut) / (1000 * 60 * 60 * 24))
        );
      ...
      if (today >= dateDebut && today <= dateFin) {
        ...
        ctx.fillText("Aujourd'hui", xToday + 5, config.margin.top - 20);
      }

      // Télécharger l'image
      const dataUrl = canvas.toDataURL("image/png");
      const link = document.createElement("a");
      link.download = `gantt-${ganttData.titre
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase()}.png`;
      link.href = dataUrl;
      link.click();
  ...
  return (
    <div className="w-full h-full bg-white p-4 overflow-y-auto scrollbar-thin">
      ...
      <GanttChart className=" text-blue-800 w-10 h-6 inline mr-3" />
          Diagramme de Gantt
        </span>
        <button
